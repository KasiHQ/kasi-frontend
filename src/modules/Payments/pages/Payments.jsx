    ...
    <div className="space-y-8">
      ...
      {selectedInvoice && (
        <>
          {/* Backdrop Scrim */}
          <div 
            className="fixed inset-0 z-50 bg-black/40 backdrop-blur-2xs transition-opacity duration-300 animate-in fade-in"
            onClick={() => setSelectedInvoice(null)}
          />
          {/* Off-canvas Panel */}
          <div className="fixed top-0 right-0 h-full w-[430px] max-w-[95vw] bg-white dark:bg-gray-900 border-l border-gray-200 dark:border-gray-800 rounded-l-3xl shadow-2xl z-50 flex flex-col overflow-hidden transition-transform duration-300 ease-out transform translate-x-0 animate-in slide-in-from-right duration-300">
            {/* Header */}
            <div className="p-6 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between select-none shrink-0 bg-gray-50/70 dark:bg-gray-800/40">
              <div>
                <h2 className="text-base font-extrabold text-gray-900 dark:text-white flex items-center gap-2">
                  <CreditCard size={18} className="text-[#1C774E] dark:text-[#DBF361]" />
                  Transaction Breakdown
                </h2>
                <p className="text-xs font-mono text-gray-400 dark:text-gray-500 mt-0.5">Reference: {selectedInvoice.reference}</p>
              </div>
              <button 
                onClick={() => setSelectedInvoice(null)}
